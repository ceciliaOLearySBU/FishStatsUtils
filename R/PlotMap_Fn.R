      ...
      if( is.null(MappingDetails) ){
        ...
        if( Rotate==0 ){
          ...
          if( map_style=="rescale" ){
            ...
            map(MappingDetails[[1]], MappingDetails[[2]], ylim=mean(Ylim)+c(-0.5,0.5)*diff(Ylim), xlim=mean(MapDetails_List[["Xlim"]])+c(0.45,0.6)*diff(MapDetails_List[["Xlim"]]), plot=TRUE, fill=FALSE, mar=c(0.1,0.1,par("mar")[3],0), myborder=0 )
            tmp_points = PlotDF[,c('Lon','Lat')]
            tmp_points[which(tmp_points$Lon < 0),"Lon"] = tmp_points[which(tmp_points$Lon < 0),"Lon"] + 360
            points(x=tmp_points[Which,'Lon'], y=tmp_points[Which,'Lat'], col=Col[Col_Bin], cex=Cex, pch=pch)
